...
class SwipeController: ItemTouchHelper.Callback {
    ...
    constructor(listener: ItemTouchHelperListener) {
    ...
    private fun drawButtons(
        ...
        if (buttonsShowedState === Button.VISIBLE) {
            ...
            p.color = Color.parseColor("#FDA4BA") //Paint 객체 컬러 지정, setColor 메서드는 int형 인자를 매개변수로 받기 때문에 string 형태의 색상을 Color 클래스의 parseColor 메서드를 이용해 int형으로 바꿔줌
            c.drawRoundRect(leftButton, corners, corners, p) //drawRoundRect : 원 그리기
            drawText("삭제", c, leftButton, p)
            buttonInstance = leftButton
        } else if (buttonsShowedState === Button.VISIBLE) { //왼쪽으로 스와이프 했을 때, 오른쪽 버튼(삭제)이 나와야 함
            val rightButton = RectF(
                itemView.right - buttonWidthWithOutPadding, (itemView.top + 10).toFloat(),
                (itemView.right - 10).toFloat(), (itemView.bottom - 10).toFloat()
            )
            p.color = Color.parseColor("FDA4BA")
            c.drawRoundRect(rightButton, corners, corners, p)
            drawText("수정", c, rightButton, p)
        ...
        recyclerView.setOnTouchListener { view, event ->
            ...
            if (swipeBack) {
                if (dX < -buttonWidth) buttonsShowedState =
                    Button.VISIBLE else if (dX > buttonWidth) buttonsShowedState =
                    Button.VISIBLE
                if (buttonsShowedState !== Button.GONE) {
    ...
    private fun setTouchUpListener(
        ...
        recyclerView.setOnTouchListener { v, event ->
            ...
            if (buttonInstance != null && buttonInstance!!.contains(
                ...
                } else if (buttonsShowedState === Button.VISIBLE) {
                    listener.onRightClick(viewHolder.adapterPosition, viewHolder)
                }
            }
            buttonsShowedState = Button.GONE
